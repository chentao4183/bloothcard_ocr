...
class BindingDialog(tk.Toplevel):
    def __init__(
        ...
        ttk.Label(self, text=f"来源: {card_info.get('source', '未知')}").pack(anchor="w", padx=10, pady=(0, 0))
        # 显示设备信息
        device_name = card_info.get('device_name')
        device_type_name = card_info.get('device_type_name')
        if device_name or device_type_name:
            device_text = []
            if device_name:
                device_text.append(f"内镜名称: {device_name}")
            if device_type_name:
                device_text.append(f"内镜类型: {device_type_name}")
            ttk.Label(self, text=" | ".join(device_text)).pack(anchor="w", padx=10, pady=(0, 10))
...
class App:
    ...
    def on_connect(self) -> None:
        ...
        self.append_log(f"已选择设备：{display_name}，开始通过蓝牙设备接收数据。")
        self.root.after(0, lambda: self.disconnect_button.configure(state=tk.NORMAL))
    ...
    def _collect_field_values(self) -> Dict[str, str]:
        ...
        for field in self.config.ocr_fields:
            ...
            # 性别转换：男->0，女->1
            if field.param_name == "sex":
                if value == "男":
                    value = "0"
                elif value == "女":
                    value = "1"
                if original_value != value:
                    self.append_log(f"[调试] 性别转换: {original_value} -> {value}")
    ...
    def _start_workflow(self, card: Dict[str, str]) -> None:
        ...
        if self.config.service.selected_version == "v2":
            ...
            if self.config.service.enable_verification:
                ...
                self._perform_ocr_and_continue(card, None, None, None, None)
        else:
            # 其他版本保持原有逻辑
                    ...
                    if data and isinstance(data, dict):
                        ...
                        if status and isinstance(status, dict):
                            ...
                            if first_status == "可用":
                                ...
                                # 从返回内容中获取endoscopeId和taskId
                                endoscope_id = data.get("endoscopeId")
                                task_id = data.get("taskId")
                                # 提取设备名称和类型名称
                                device_name = data.get("name")
                                device_type_name = data.get("typeName")
                                
                                self.append_log(f"[V2版本] 获取到endoscopeId: {endoscope_id}, taskId: {task_id}")
                                self.append_log(f"[V2版本] 获取到设备信息: name={device_name}, typeName={device_type_name}")
                                
                                # 验证合格后执行OCR识别，并传递endoscopeId、taskId、设备名称和类型名称
                                self._perform_ocr_and_continue(card, endoscope_id, task_id, device_name, device_type_name)
                                return
    ...
    def _perform_ocr_and_continue(self, card: Dict[str, str], endoscope_id: str = None, task_id: str = None, device_name: str = None, device_type_name: str = None) -> None:
        """执行OCR识别并继续后续流程"""
        try:
            # 执行OCR识别所有字段
            for field in self.config.ocr_fields:
            ...
            # 构建完整payload，包含endoscopeId、taskId、device_name和device_type_name
            payload = {
                "card_hex": card.get("hex"),
                "card_dec": card.get("dec"),
                "timestamp": _human_now(),
                "fields": field_values,
                "endoscopeId": endoscope_id,
                "taskId": task_id,
    ...
    def _open_binding_dialog(self, payload: Dict) -> None:
        ...
        device_type_name = payload.get("device_type_name")
        card_info = {"hex": payload.get("card_hex", ""), "dec": payload.get("card_dec", ""), "source": source, "device_name": device_name, "device_type_name": device_type_name}
        auto = self.config.backend.submission_mode == "auto"
        seconds = self.config.backend.auto_delay_seconds
        
        # 提取field_values并添加endoscopeId和taskId
        field_values = payload.get("fields", {})
        
        # 添加endoscopeId和taskId到field_values中显示
        if "endoscopeId" in payload:
            field_values["endoscopeId"] = payload["endoscopeId"]
        if "taskId" in payload:
            field_values["taskId"] = payload["taskId"]
    ...
    def _submit_binding_payload(self) -> None:
        ...
        # 重新构建提交数据格式，只包含fields里的内容和endoscopeId、taskId
        submit_data = {}
        
        # 添加fields里的所有内容
        fields = self.pending_binding_payload.get("fields", {})
        submit_data.update(fields)
        
        # 添加endoscopeId和taskId（如果存在）
        if "endoscopeId" in self.pending_binding_payload:
            submit_data["endoscopeId"] = self.pending_binding_payload["endoscopeId"]
        if "taskId" in self.pending_binding_payload:
            submit_data["taskId"] = self.pending_binding_payload["taskId"]
        
        # 打印即将提交的数据到日志
        self.append_log(f"[提交数据] 即将提交到信息绑定接口的数据: {json.dumps(submit_data, ensure_ascii=False)}")
    ...
    def _on_binding_success(self, data: Dict) -> None:
        ...
        # 检查返回数据中的msg字段是否包含"成功"关键字
        msg_content = data.get("msg", "") if isinstance(data, dict) else ""
        
        # 从pending_binding_payload中获取DJName和name的值
        dj_name = ""
        name = ""
        if self.pending_binding_payload:
            if "fields" in self.pending_binding_payload:
                fields = self.pending_binding_payload["fields"]
                dj_name = fields.get("name", "")
            # 使用device_name作为name值，与"信息绑定确认"弹窗中的内镜名称保持一致
            name = self.pending_binding_payload.get("device_name", "")
        ...
        # 根据msg字段内容决定弹窗显示内容
        if "成功" in msg_content:
            messagebox.showinfo("信息绑定成功", f"{dj_name}和{name}绑定成功")
        else:
    ...
    def _post_request(self, url: str, payload: Dict, on_success, on_error) -> None:
        ...
        def _callback(fut):
            ...
            # 记录POST请求的返回信息
            if ok:
                self.append_log(f"[信息绑定接口] 请求成功，返回数据: {json.dumps(result, ensure_ascii=False)}")
            else:
                self.append_log(f"[信息绑定接口] 请求失败，返回数据: {json.dumps(result, ensure_ascii=False)}")
    ...
    def _on_hid_card(self, value: str, device_name: str) -> None:
        ...
        def _handle() -> None:
            ...
            # 记录所有设备的数据
            self.append_log(f"[调试] 检测到设备数据: {value}")
            ...
            # 对所有设备使用统一的处理逻辑
            self.append_log("[调试] 应用统一处理规则到设备数据")
            # 尝试从原始数据中提取数字部分
            ...
            # 使用HID设备作为来源
            card = {"hex": hex_value, "dec": dec_value, "source": f"HID:{device_name}"}
            self.append_log(f"HID刷卡：10D={dec_value} 来自 {device_name}")

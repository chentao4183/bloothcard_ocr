...
class HidConfig:
    """HID监听配置"""
    enabled: bool = True
    device_keywords: List[str] = field(default_factory=lambda: [])
    digit_length: int = 10
...
class AppConfig:
    ...
    def default(cls) -> "AppConfig":
        fields = [
            OCRField(field_id=str(uuid.uuid4()), name="卡ID", param_name="endoscopeId", builtin=True),
            OCRField(field_id=str(uuid.uuid4()), name="诊疗时间", param_name="Treatime", builtin=True),
            OCRField(field_id=str(uuid.uuid4()), name="唯一ID", param_name="patientNum", sample_value="ID001"),
            OCRField(field_id=str(uuid.uuid4()), name="流水号", param_name="diagnosisNum", sample_value="SN001"),
            OCRField(field_id=str(uuid.uuid4()), name="病人姓名", param_name="name", sample_value="张三"),
            ...
            OCRField(field_id=str(uuid.uuid4()), name="医生", param_name="diagnosisDoctor", sample_value="王医生"),
            OCRField(field_id=str(uuid.uuid4()), name="护士", param_name="auxiliaryNurse", sample_value="李护士"),
            OCRField(field_id=str(uuid.uuid4()), name="诊疗间", param_name="ediagnosisRoom", sample_value="诊疗间2"),
...
class ConfigManager:
    ...
    def save(self, config: AppConfig) -> None:
        ...
        self.path.write_text(serialized, encoding="utf-8")#哈哈哈哈哈哈
